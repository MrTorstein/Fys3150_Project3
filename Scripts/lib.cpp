...
void lib::gauss_laguerre(double *x, double *w, int n, double alf)
{
    int i,its,j;
    double ai;
...
double lib::Funk(double x1, double y1, double z1, double x2, double y2, double z2)
{
    if(fabs(x1 + y1 + z1 - (x2 + y2 + z2)) < pow(10, -10))
    {
        return 0;
    }
    else
    {
        return 1 / fabs(x1 + y1 + z1 - (x2 + y2 + z2)) * exp(-2 * 2 * (sqrt(x1 * x1 + y1 * y1 + z1 * z1) + sqrt(x2 * x2 + y2 * y2 + z2 * z2)));
    }
}
